 ...
 * `routes/recorder.js` and the export/traceability routes to
 * `routes/testExports.js`. The remaining concerns here — CRUD, AI
 * generation, single-test runs, review/approvals, visual baselines — are
 * a candidate for further extraction (`routes/testApprovals.js`). The
 * route-order constraint ("bulk must be declared BEFORE `:testId`
 * wildcards") still applies and is easy to regress in a mechanical move.
 *
 * ### Endpoints
