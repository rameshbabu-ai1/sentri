 ...
 * `routes/recorder.js`. The remaining concerns here — CRUD, AI generation,
 * single-test runs, review/approvals, visual baselines, exports — are
 * candidates for further extraction (`routes/testExports.js`,
 * `routes/testApprovals.js`). The route-order constraint ("bulk must be
 * declared BEFORE `:testId` wildcards") still applies and is easy to
 * regress in a mechanical move.
 *
 * ### Endpoints
