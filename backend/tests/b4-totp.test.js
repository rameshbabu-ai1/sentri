...
const ctx = createTestContext("b4-totp");
...
test("base32Decode tolerates whitespace + lowercase + padding", () => {
...
test("generateTotpSecret yields a 32-char base32 string", () => {
  const secret = generateTotpSecret();
  assert.match(secret, /^[A-Z2-7]{32}$/);
});

test("computeTotpAtStep matches RFC 6238 vector at T=59 (step=1)", () => {
  // RFC 6238 Appendix B, step counter = 1 → expected "287082"
  const code = computeTotpAtStep(RFC_SECRET_B32, 1);
  assert.equal(code, "287082");
});

test("computeTotpAtStep matches RFC 6238 vector at T=1111111109 (step=37037036)", () => {
  // RFC 6238 Appendix B → expected "081804"
  const code = computeTotpAtStep(RFC_SECRET_B32, 37037036);
  assert.equal(code, "081804");
});

test("generateTotpCode returns { code, expiresInSeconds } and code verifies", () => {
  const secret = generateTotpSecret();
...
test("verifyTotp rejects malformed input (non-6-digit, empty, alpha)", () => {
  const secret = generateTotpSecret();
...
test("verifyTotp accepts ±1 window of clock skew by default", () => {
  const secret = generateTotpSecret();
...
test("verifyTotp rejects codes from outside the ±1 window", () => {
  const secret = generateTotpSecret();
...
test("encryptCredentials round-trips totpSecret through AES-256-GCM", () => {
...
test("encryptCredentials omits totpSecret cleanly when not provided", () => {
  const enc = encryptCredentials({
    username: "alice@example.com",
    password: "p4ssw0rd!",
  });
...
test("decryptCredentials handles legacy rows (no _encrypted) without throwing", () => {
