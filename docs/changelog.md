...
- **B3** — Two new Prometheus counters: `app_agent_reviewer_collapsed_total{projectId}` bumps once per run when the collapse gate fires; `app_review_rejections_total{projectId}` bumps per discarded test. Both labelled by `projectId` so multi-tenant operators alert on the per-project slice from Prometheus alone (mirrors the `app_run_p95_load_ms{projectId}` / `app_vision_heal_budget_exhausted_total{projectId, reason}` pattern). Very large multi-tenant deployments can `relabel_configs`-drop the label at scrape time.
