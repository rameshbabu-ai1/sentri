...
- **B4** — Target-app MFA support: projects can now store an AES-256-GCM-encrypted RFC 6238 TOTP seed alongside username + password. `pipeline/autoLogin.js` detects an OTP field after the password submit and fills the live 6-digit code automatically, with a one-time retry on a 30-second window-boundary rejection. Industry-standard detection waterfall: `autocomplete="one-time-code"` (HTML spec) → `aria-label`/`placeholder`/`name` containing "code" / "OTP" / "verification".
- **B4** — Admin-only `POST /api/v1/projects/:id/credentials/test-totp` preview endpoint returns the live 6-digit code + `expiresInSeconds` so operators can verify their seed matches the target app's enrollment without ever exposing the seed over the wire (mirrors Auth0 / Okta "Test connection" UX). Audit-logged as `project.credentials.test_totp`.
- **B4** — Per-project `sessionRefreshIntervalMs` setting (60_000 – 86_400_000 ms) enables proactive session keep-alive pings for apps with short idle timeouts; `null` (default) keeps the legacy no-ping behaviour bit-for-bit.
- **B4** — `auth_expired` skip reason added to `NON_EXECUTED_SKIP_REASONS` (backend + frontend in lockstep) so environmental session-expiry failures are excluded from the pass-rate denominator and never trigger `maxFailures` gate violations. Mirrors the same accounting contract as `over_budget` (AUTO-001) and `upstream_failed` (AUTO-014).
- **B4** — Feedback-loop classifier recognises `AUTH_EXPIRED` failures (`auth_session_expired_unrecoverable`, `relogin_failed`, `MFA session expired`) and excludes them from auto-regeneration — re-prompting the LLM can't fix an expired cookie. The Quality Insights panel will surface the honest message instead of the misleading "selectors are broken" insight.
- **B4** — Shared `utils/totp.js` module extracted from `routes/auth.js` so SEC-004's own MFA flow and the new target-app login helper share ONE algorithm. Includes RFC 6238 reference test-vector coverage at `backend/tests/b4-totp.test.js`.
