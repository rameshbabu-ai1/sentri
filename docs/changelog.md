...
- **B3** — Two new Prometheus counters: `app_agent_reviewer_collapsed_total` bumps once per run when the collapse gate fires; `app_review_rejections_total` bumps per discarded test. Both are unlabelled — dashboards alert on platform-wide rate without cardinality concerns.
