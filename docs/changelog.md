...
- **B2** — Crawler now enumerates same-origin iframes per page; embedded widgets (payment forms, Intercom, Typeform, Stripe Checkout) are visible to test generation. Per-project `iframeStrategy` controls scope: `same-origin` (default), `allowlist`, `all`, or `none`.
- **B2** — SPA hydration wait before snapshotting: React/Vue/Angular/Next.js apps no longer get captured in skeleton state. Per-project `hydrationType` switches between `auto` (default — waits for common loading indicators), `domcontentloaded` (opt-out), and `custom` (operator-supplied selector).
- **B2** — Adaptive element timeout per run: derived from `2 × p95(crawl page-load ms)` clamped to `[HEALING_ELEMENT_TIMEOUT, MAX_ELEMENT_TIMEOUT]`. Enterprise apps with 8–15s API calls no longer burn the 7-stage healing waterfall on TIMEOUT failures. Per-project `elementTimeoutOverride` is the operator escape hatch.
- **B2** — `runs.p95LoadMs` column persisted at run-start so RunDetail / dashboards / CI consumers can audit the value the adaptive calculation used.
- **B2** — Four new Prometheus metrics expose B2 behaviour to operator dashboards: `app_run_p95_load_ms{projectId}` gauge, `app_run_adaptive_timeout_ms{projectId, source}` gauge (source ∈ `project_override` / `adaptive` / `default`), `app_iframe_enumerated_total{strategy, outcome}` counter (outcome ∈ `captured` / `skipped_cross_origin` / `skipped_strategy` / `error`), `app_spa_hydration_wait_seconds{mode}` histogram. Operators can now alert on a rising p95LoadMs (latent app regression) or a sustained cross-origin-iframe-skip rate.
- **B3** — Pre-run reviewer-collapse gate: when the workspace's `author` and `reviewer` agent roles resolve to the same provider route, the LLM reviewer pass is skipped and the run uses heuristic-only review. Surfaced as a `Reviewer collapsed — heuristic-only review` chip on RunDetail and a warning banner in Settings → Agent Roles.
- **B3** — Per-test review-rejection escalation: when the reviewer↔author loop discards a candidate via `ReviewRejection`, the run accumulates the discard on `run.reviewRejectedTests[]`, a `test.review_rejected` activity row lands in the audit log, and FEA-001 channels (Teams / email / webhook) escalate per the new `reviewRejectionAlertThreshold` per-project setting (default 0 = always notify; -1 = opt-out).
- **B3** — RunDetail "Tests discarded by review" section lists each rejected test with its failure category, rounds completed, and a deep link to TestDetail.
- **B3** — Three new Prometheus counters: `app_agent_reviewer_collapsed_total{projectId}` bumps once per run when the collapse gate fires; `app_review_rejections_total{projectId}` bumps per discarded test; `app_review_rejection_notifications_total{channel, outcome}` bumps per notification dispatch (channel ∈ {teams, email, webhook}, outcome ∈ {sent, failed, cooldown_skipped, threshold_skipped, disabled, no_settings}) so operators can SLO against the integration itself, not just the source signal. First two labelled by `projectId` for per-project alert routing (mirrors `app_vision_heal_budget_exhausted_total{projectId, reason}` / `app_run_p95_load_ms{projectId}`); deployments at extreme scale can `relabel_configs`-drop the label.
- **B3** — Per-project notification cooldown via `projects.reviewRejectionAlertLastFiredAt` (migration 071). The dispatcher debounces channel writes for `REVIEW_REJECTION_NOTIFICATION_COOLDOWN_MS` (default 1 h, env-tunable to `0` for opt-out) so a project that discards 50 tests across 10 runs in a minute doesn't fire 10 Teams notifications. Mirrors the existing `workspaces.spendAlertLastFiredAt` pattern.
- **B3** — Failed notification dispatches enqueue to the existing `audit_dlq` table for admin replay via the SIEM DLQ inspector. A transient Teams outage no longer loses the audit trail of "we tried to alert about run X but couldn't".
- **B3** — Three new Prometheus alerts in `monitoring/prometheus/alerts.yml`: `ReviewerCollapseSpike` (>5 collapsed runs/h per project), `ReviewRejectionRateHigh` (>20% rejection rate over 10 min per project), `ReviewRejectionNotificationFailureRate` (>10% notification delivery failure rate over 15 min per channel). All three carry `runbook_url` annotations pointing at `docs/guide/observability.md`. `ReviewRejectionRateHigh` uses the new `projectId` label on `app_run_outcome_total` (added in this PR) as the per-project denominator.
- **B4** — Tests no longer fail-hard when the target app's session expires mid-run; the runner detects login-page redirects after each navigation and replays `username + password (+ TOTP)` against the project's stored credentials before retrying the action.
- **B4** — Target-app MFA support — projects can store an AES-256-GCM-encrypted RFC 6238 TOTP seed alongside username + password, and `pipeline/autoLogin.js` auto-fills the live 6-digit code after the password submit with a one-time retry on 30-second window-boundary rejections.
- **B4** — TOTP field detection follows the industry-standard waterfall: `autocomplete="one-time-code"` first (HTML spec / Apple Keychain / Chrome autofill), then `aria-label`/`placeholder`/`name` containing "code", "OTP", or "verification".
- **B4** — Admin-only `POST /api/v1/projects/:id/credentials/test-totp` preview returns the live code + `expiresInSeconds` so operators can verify their seed without ever exposing it over the wire (mirrors Auth0 / Okta "Test connection" UX), audit-logged as `project.credentials.test_totp`.
- **B4** — Per-project `sessionRefreshIntervalMs` (60_000 – 86_400_000 ms) enables proactive session keep-alive pings via a second `BrowserContext` page; `null` (default) keeps the legacy no-ping behaviour bit-for-bit identical.
- **B4** — `auth_expired` skip reason added to `NON_EXECUTED_SKIP_REASONS` (backend + frontend) so environmental session expiries are excluded from the pass-rate denominator and never trigger `maxFailures` gate violations (same contract as `over_budget` / `upstream_failed`).
- **B4** — Feedback-loop classifier recognises `AUTH_EXPIRED` failures (`auth_session_expired_unrecoverable`, `relogin_failed`, `MFA session expired`) and excludes them from auto-regeneration because re-prompting the LLM can't fix an expired cookie.
- **B4** — Quality Insights panel now surfaces the honest "session expired" message on auth-expired failures instead of the misleading "selectors are broken" insight that previously fired.
- **B4** — Shared `utils/totp.js` module extracted from `routes/auth.js` so SEC-004's own MFA flow and the new target-app login helper share one RFC 6238 implementation, with reference-vector coverage at `backend/tests/b4-totp.test.js`.
- **B4** — Industry-standard consumer-side TOTP seed policy: 16–25 char seeds accepted with `weakTotpSeed` audit-log signal (sub-RFC 4226), 26+ char seeds accepted silently (matches 1Password / Authy / `oathtool` behaviour against third-party SUTs).
- **B4** — `restoreAuthSession` now distinguishes `credentials_decryption_failed` (AES round-trip failed) from `credentials_blank` (decrypt OK but fields empty) so operator diagnostics point at the real fix path instead of a false key-rotation lead.
- **B6 / QAL-001** — Per-project opt-in dry-run gate executes every generated test once via `browserPool` BEFORE it enters the review queue. Outcomes (`passed` / `failed` / `trivial`) land on `tests.dryRunStatus` + `dryRunError` + `dryRunDurationMs` (migration 073); a dry-run failure is NEVER auto-approved per AUTO-003b. Toggle via `projects.dryRunGate` (default off — pipeline byte-identical to pre-B6 on disabled projects).
- **B6 / QAL-001** — Trivial-assertion detection: tests that pass in < `DRY_RUN_TRIVIAL_THRESHOLD_MS` (default 200 ms) with zero network requests are flagged `dryRunStatus: 'trivial'` so operators can spot smoke-only generations before approving.
- **B6 / QAL-005** — Second-pass LLM semantic reviewer asks four contract questions per generated test (meaningful state change, trivially-true assertions, full scenario coverage, regression-catching value) and persists a structured `{ score, verdict, issues[] }` verdict on `tests.semanticReviewScore` + `semanticReviewIssues` (migration 074). `reject` verdict flips `reviewStatus = 'rejected'`. Silently no-ops when the B3 reviewer-collapse gate has fired (same model on both ends has no independent signal).
- **B6 / QAL-002** — Per-test `setupCode` + `teardownCode` columns (migration 075) emitted by the journey + intent prompts. Setup runs BEFORE the test body; teardown runs in a `finally` block with errors swallowed + logged so cleanup never masks a real test failure. Generated tests that create resources (signup, order, file upload) now include teardown via the same UI flow, removing the "tests pass first run, fail on re-run" UNIQUE-constraint footgun.
- **B6 / QAL-010** — Seeded faker substitution for unique test data: prompts emit `__FAKE_EMAIL__` / `__FAKE_NAME__` / `__FAKE_PHONE__` / `__FAKE_UUID__` (15 tokens total); the runner replaces them with deterministic `@faker-js/faker` values seeded from `hash(runId + testId)` — same retry sees the same values, different runs see different values so re-running on the same DB doesn't trip UNIQUE constraints. `@faker-js/faker` is an OPTIONAL backend dep; missing it degrades to a deterministic SHA-256 fallback so CI without it stays functional.
- **B6 / QAL-010** — Per-project `testDataLocale` setting (migration 076, default `'en'`) drives the faker locale registry so generated test data matches the target market. 15 locales supported (`en_US`, `en_GB`, `es_MX`, `fr_CA`, `ja`, `zh_CN`, …); typo lands a 400 at the route layer instead of silent fallback.
- **B6** — Prompt version bumped 2.4.0 → 2.5.0 to track the new optional `setupCode` / `teardownCode` fields + faker-token convention. `tests.promptVersion` stamps make pre-B6 vs post-B6 generations distinguishable for A/B quality analysis.
- **CI** — `promtool check rules monitoring/prometheus/alerts.yml` job added to `.github/workflows/ci.yml`. Catches typos / malformed PromQL / missing labels before alerts ship to production and silently disable themselves.
