...
- **B1.1 (AUDIT-ROADMAP)** — New `run_test_results` append-only table flushes per-test outcomes the moment they complete (migration `062`), so a SIGKILL / OOM / container kill mid-run preserves every result collected up to that point instead of dropping the in-memory `run.results[]`. `ON DELETE CASCADE` on `runId` mirrors `run_logs` (SOC 2 CC8.1 audit-trail integrity).
- **B1.1 (AUDIT-ROADMAP)** — Duplicate-write observability: `runTestResultRepo.append()` bumps `app_run_test_result_duplicates_total{reason}` on every UNIQUE-constraint hit and emits a structured warn log on unexpected `duplicate_dispatch` hits, so silent drops are alertable. Splunk / Datadog / Auth0 convention.
- **B1.2 (AUDIT-ROADMAP)** — Tiered-durability write queue (`utils/dbWriteQueue.js`) collapses high-frequency SQLite writes into single transactions, lifting effective throughput under `parallelWorkers > 1`. Two modes: `"batched"` (default — < 1 ms enqueue, loses up to one batch on SIGKILL) and `"durable"` (synchronous commit, lose-nothing). Matches the Postgres `synchronous_commit` / Kafka `acks` / MySQL `sync_binlog` industry pattern. Tuneable via `DB_WRITE_BATCH_SIZE` (default 50) and `DB_WRITE_FLUSH_MS` (default 100). PostgreSQL deployments are a passthrough.
- **B1.3 (AUDIT-ROADMAP)** — New `crawl_snapshots` table (migration `063`) lets the crawler stream snapshots to disk as each page finishes instead of accumulating them in heap, with a `loadMs` column the upcoming Bundle 2 adaptive-timeout work consumes. `ON DELETE CASCADE` on `runId`.
- **B1.3 (AUDIT-ROADMAP)** — `crawlBrowser.js` and `stateExplorer.js` now persist each crawled page via `crawlSnapshotRepo.save()` immediately after the structure-dedup gate. The legacy in-memory `snapshots[]` accumulation is kept as a shadow path during the B1 → B2 transition so downstream pipeline stages keep their current contract; peak heap on a 200-page crawl drops to O(1 page) once consumers flip to the repo as the source of truth.
- **B1 (AUDIT-ROADMAP)** — New `runs.failureReason` and `runs.reviewRejectedTests` columns (migration `064`) capture process-crash vs ordinary failure attribution. The boot-time orphan recovery (`markOrphansInterrupted`) now stamps `failureReason='process_crash'` so resume + audit consumers can distinguish a SIGKILL / OOM kill from a user abort or ordinary failure.
- **B1 (AUDIT-ROADMAP)** — `POST /api/v1/runs/:runId/resume` (admin-only) re-dispatches only the tests missing from `run_test_results` so the operator picks up where the server crash left off instead of restarting from scratch. Mirrors GitHub Actions `re-run failed jobs` / CircleCI `rerun-from-failed` / Buildkite `retry job`. RunDetail surfaces a `Resume` button + an `Interrupted` badge for crash-recovered runs.
